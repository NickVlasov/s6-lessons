...
def load_dataset_file_to_vertica(
    ...
    vertica_conn = vertica_python.connect(
        ...
        port=5433,
        user='vt260912b723bf',
        password='b6648e0c25c94f5490e8ea78e0623b58'
    )
    columns_str = ', '.join(columns)
    copy_expr = f"""
    COPY {schema}.{table} ({columns_str}) FROM STDIN DELIMITER ',' ENCLOSED BY '"'
    """
    ...
    with contextlib.closing(vertica_conn.cursor()) as cur:
        ...
        while start <= num_rows:
            ...
            df.loc[start: end].to_csv('/tmp/chunk.csv', index=False, header=False)
...
def sprint6_project_dag_load_data_to_staging():
    ...
    load_group_log = PythonOperator(
        ...
        op_kwargs={
            ...
            'schema': 'VT260912B723BF__STAGING',
            'table': 'group_log',
            'columns': ['group_id','user_id','user_id_from','event','datetime'],
            'type_override': {'group_id': 'Int64', 'user_id_from': 'Int64', 'user_id': 'Int64'},
        },
    )
